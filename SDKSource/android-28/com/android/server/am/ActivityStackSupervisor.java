...
public class ActivityStackSupervisor extends ConfigurationContainer implements DisplayListener,
    ...
    /**
     * 兜兜转转又回到了ActivityStackSupervisor的startSpecificActivityLocked的这个方法
     * 这个方法很重要。这个方法将是普通Activity和根Activity启动流程的分岔路口。
     */
    void startSpecificActivityLocked(ActivityRecord r,
        ...
        // 判断当前的启动Activity的所在的Application的是否已经运行
        // 可以知道在方法中首先获取到了即将要启动的Activity所在的应用进程，
        // 假如是普通的Activity的启动流程的活，这个进程肯定是存在的，所以将执行realStartActivityLocked的方法。
        ...
        // 如果是根Activity的启动流程，由于应用都还未启动，
        // 意味着根Activity所在的应用进程还未创建，而mService其实就是AMS，
        // 所以这里将调用AMS的startProcessLocked。于是我们又回到了最初的起点AMS
        // 所以我们再回到ActivityManagerService看startProcessLocked的执行
    ...
    /**
     * 在启动Activity的过程中。ActivityManagerService在启动会逐步调用
     * ActivityStackSupervisor的resumeFocusedStackTopActivityLocked
     * 这个方法准备激活当前栈的待启动的Activity
     */
    ...
    boolean resumeFocusedStackTopActivityLocked(
        ...
        if (r == null || !r.isState(RESUMED)) {
            // resumeFocusedStackTopActivityLocked 又调用了resumeTopActivityUncheckedLocked
            // 那我继续看当前的这个ActivityStack的resumeTopActivityUncheckedLocked
