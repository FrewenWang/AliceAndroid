...
class ActivityStarter {
    ...
    int execute() {
        try {
            ...
            // 因为在startActivityAsUser的链式方法中我们调用了setMayWait这个方法，所以这里的mRequest.mayWait为true
            if (mRequest.mayWait) {
                // 我们看这个地方就是直接调用startActivityMayWait的方法
    ...
    /**
     * startActivityMayWait方法经过调用多次的startActivity方法后会调用到startActivityUnchecked这个方法
     */
    private int startActivityMayWait(IApplicationThread caller, int callingUid,
        ...
        synchronized (mService) {
            ...
            // 方法执行到这个地方我们可以看到执行了ActivityStarter执行startActivity
    ...
    private int startActivity(final ActivityRecord r, ActivityRecord sourceRecord,
        ...
        try {
            ...
            // 那这个方法是干啥的呢？
            // 这个方法会根据启动标志位和Activity启动模式来决定如何启动一个Activity
            // 我们来继续看这个方法的实现
    ...
    /**
     * 这个方法会根据启动标志位和Activity启动模式来决定如何启动一个Activity
     * 以及是否要调用deliverNewIntent方法通知Activity有一个Intent试图重新启动它。
     */
    ...
    private int startActivityUnchecked(final ActivityRecord r, ActivityRecord sourceRecord,
        ...
        if (mDoResume) {
            ...
            if (!mTargetStack.isFocusable()
                ...
                // 从代码中我们可以看到无论是哪种模式启动
                // 最终都会调用ActivityStackSupervisor.resumeFocusedStackTopActivityLocked方法。
                // 下面我们来看一下这个方法的实现
