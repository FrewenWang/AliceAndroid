                                  ...
                                  , AudioRouting
{
     ...
     * 播放器的默认构造方法。播放的默认构造方法调用之后
     * 播放器实例被创建。这个时候播放器处于Idle的状态
     * 
     * 
     */
    public MediaPlayer() {
        ...
        // 声明一个Looper对象
        Looper looper;
        // 获取当前线程的Looper对象。如果当前线程的Looper对象为null
        // 则我们获取主线程的Looper对象
        ...
        // 这是时间数据容器。
        // 我们可以作为一个编程规范：一般Provider都是和数据提供有关的
        // VideoProvider、ContentProvider
    ...
    public void setDisplay(SurfaceHolder sh) {
        ...
        // 这个地方就是给视频设置surface。调用的是一个Native方法
     ...
     * @return a MediaPlayer object, or null if creation failed
     * MediaPlayer对外提供的静态的Create()方法。这个方法里面包含MediaPlayer的对象的实例化
     * setsetDataSource()方法
     * prepare()方法
     */
    public static MediaPlayer create(Context context, Uri uri, SurfaceHolder holder,
        ...
        try {
            ...
            // 然后将声音属性设置给MediaPlayer
            mp.setAudioAttributes(aa);
            // 设置声音的会话ID。视频和音频是分开渲染的
            mp.setAudioSessionId(audioSessionId);
            // 从这里开始setDataSource 
            mp.setDataSource(context, uri);
            // 判断Surface是否为空，这是一个控制器
            if (holder != null) {
                // Surface控制器，用来操控Surface,处理他在Canvas上作画的动画和效果
                // 从而处控制表面、大小、像素等
                // 给Surface设置一个Surface控制器
                mp.setDisplay(holder);
            }
            // 调用MediaPlayer的prepare方法
            mp.prepare();
            return mp;
    ...
    /**
     * 设置播放的数据资源
     * 这个方法将传递一个播放器的Idle的状态变成Initial状态。如果不是这个状态调用
     * setDataSource的话，会报IllegalArgumentException
     */
    @UnsupportedAppUsage
     ...
     * 在给播放器设置数据源且展现了Surface后，你应该开始调用prepare和prepareAsync函数
     * 对于文件类型，调用prepare是会阻塞，prepare是一个方法
     *
     * @throws IllegalStateException if it is called in an invalid state
     */
     ...
     *
     * @throws IllegalStateException if it is called in an invalid state
     * 当播放器进入prepared状态之后，也就是回调onPrepared的方法之后，这个时候我们就可以调用
     * start()函数并成功返回
     */
     ...
     * at the same time.
     * 
     * 播放器的release。播放的release方法调用之后
     * 播放器实例被创建。这个时候播放器处于END的状态
     * 从Idle状态到End的状态之后的播放状态就是播放器完整的生命周期
     */
    public void release() {
        baseRelease();
        stayAwake(false);
            ...
            throws IllegalStateException
    {
        ...
        // 这个地方是创建HandlerThread的一个子乡城
        final HandlerThread thread = new HandlerThread("SubtitleReadThread",
              Process.THREAD_PRIORITY_BACKGROUND + Process.THREAD_PRIORITY_MORE_FAVORABLE);
        thread.start();
        // 获取子线程的Looper对象。实例化含有这个Looper对象的Handler
        Handler handler = new Handler(thread.getLooper());
        // Handler进行
        handler.post(new Runnable() {
    ...
    private class EventHandler extends Handler
    {
        ...
        public void handleMessage(Message msg) {
            ...
            case MEDIA_PREPARED:
                ...
                // 当播放器收到MEDIA_PREPARED的事件时候，回调给上层onPrepared
            ...
            case MEDIA_BUFFERING_UPDATE:
                // 当我们的播放器调用Start之后，MediaPlayer就从prepared的状态进入到Started状态中
                // 这个时候播放器就处于加载缓冲的逻辑中
            ...
            case MEDIA_ERROR:
                // 播放器在生命周期内部调用任何方法出现异常，都会回调到这个方法。回调给上层客户端onError方法
