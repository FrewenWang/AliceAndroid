...
/**
 * @filename: 请求gitee服务器的API调用
 * @introduction:
 * @author: Frewen.Wong
 * @time: 2020/5/13 21:30
 * Copyright ©2020 Frewen.Wong. All Rights Reserved.
 */
class GiteeAndroidApi : AbsNetworkApi() {
    ...
    companion object {
        ...
        @Volatile
        private var sInstance: GiteeAndroidApi? = null
        val instance: GiteeAndroidApi
            get() {
                if (sInstance == null) {
                    synchronized(GiteeAndroidApi::class.java) {
                        if (sInstance == null) {
                            sInstance = GiteeAndroidApi()
                        }
                    }
                }
                return sInstance!!
